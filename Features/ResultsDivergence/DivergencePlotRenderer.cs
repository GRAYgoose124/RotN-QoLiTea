...
internal static class DivergencePlotRenderer
{
        ...
        PlotDisplayMode mode)
    {
        ...
        SyncVibeLabels(go, context, fullscreen);
        return raw;
    }
    ...
    internal static void RefreshTexture(RawImage raw, PlotRenderContext context, PlotDisplayMode mode)
    {
        ...
        if (old != null)
            Object.Destroy(old);
        SyncVibeLabels(raw.gameObject, context, fullscreen);
    }
    ...
    private const string VibeLabelsRoot = "QoLiTeaVibeLabels";

    internal static void SyncVibeLabels(GameObject plotGo, PlotRenderContext context, bool fullscreen)
    {
        if (plotGo == null)
            return;

        var existing = plotGo.transform.Find(VibeLabelsRoot);
        if (existing != null)
            Object.Destroy(existing.gameObject);

        if (!fullscreen || context?.VibeStats == null || context.VibeStats.Count == 0)
            return;

        var root = new GameObject(VibeLabelsRoot, typeof(RectTransform));
        root.transform.SetParent(plotGo.transform, false);
        ...
        for (var i = 0; i < context.VibeStats.Count; i++)
        {
            ...
            string label = $"{stat.HitCount}: {first} → {last}";
            ...
            text.color = new Color(1f, 0.92f, 0.55f, 0.95f);
    ...
    {
        if (x < 0 || x >= texW || y < 0 || y >= pixels.Length / texW)
            return;
        int i = y * texW + x;
